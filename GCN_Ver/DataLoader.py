...
class DataModule(pl.LightningDataModule):
    def __init__(self, data_path,
        ...
        self.sample(self.data.copy())
        self.generate_dataset(self.data.copy(), self.train_index.copy(), self.val_index.copy())
    ...
    def load_data(self, file_path):
        ...
        self.raw_data = dat.copy()
        dat = self.preprocess_time(dat.copy())
    ...
    def generate_dataset(self, data, train_index, val_index):
        self.generate_traditional_data(data.copy(), train_index, val_index)
        spatial_adj, spatial_feature = graph_utils.generate_spatial_graph(data.copy(), self.route_adj, self.route_mapper)
        temporal_adj, temporal_feature = graph_utils.generate_temporal_graph(data.copy(), self.raw_data.copy(), self.time_limit)
        neighbor_adj, neighbor_feature = graph_utils.generate_k_nearest_graph(data.copy(), self.k_neighbors)
...
class Data_2019(pl.LightningDataModule):
    def __init__(self, data_path,
                 node_num=343, k_neighbors=3,
                 split_ratio=0.8, delt_t=20, seed=0, time_limit=30, pure=False):
        super(Data_2019, self).__init__()
        self.np_ratio = -1
        self.graph_feature_num = -1
        self.feature_num = -1
        self.x_train = None
    ...
    def load_data(self, file_path):
        ...
        sheet_names = xls.sheet_names
        dat = None
        raw_cols = ['Date', 'Time', 'Day', 'Route', 'Delay', 'Incident']
        graph_feature_cols = ['Month', 'Time', 'Day', 'Incident']
        for sheet_name in sheet_names:
            df = pd.read_excel(file_path, sheet_name, dtype={'Report Date': str, 'Time': str})
            df = df.dropna()
            if 'Report Date' in df.columns:
                df = df.rename(columns={'Report Date': 'Date'})
            if 'Min Delay' in df.columns:
                df = df.rename(columns={'Min Delay': 'Delay'})
            # remove cols that not in target_cols
            for col in df.columns:
        ...
        dat['Time'] = pd.to_datetime(dat['Time']).dt.strftime('%H:%M:%S')
        # convert 'Delay' to 0/1 by the limit 30 if Delay >= 30
        if dat['Delay'].max() > 1:
            dat['Delay'] = (dat['Delay'] >= 30).astype(int)
        pos = 0
        ...
        print(f"negative count: {neg}")
        print(f"raw data row count: {len(dat)}")
        # set a deep copy of the raw data
        self.raw_data = dat.copy()
        dat = self.preprocess_time(dat)
        dat = pd.get_dummies(dat, columns=graph_feature_cols)
        self.graph_feature_num = len(dat.columns) - 2
        self.feature_num = len(dat.columns) - 2
        self.data_len = len(dat)
        print(f"graph feature num: {self.graph_feature_num}")
    ...
    def generate_dataset(self, data, train_index, val_index):
        routes = data['Route'].values.astype(int)
        ...
        spatial_adj, spatial_feature = graph_utils.generate_route_graph(data, routes)
    ...
    def preprocess_time(self, dat):
        assert 'Date' in dat.columns
        dat['Date'] = pd.to_datetime(dat['Date']).dt.month
        dat.rename(columns={'Date': 'Month'}, inplace=True)
        assert 'Time' in dat.columns
        dat['Time'] = pd.to_datetime(dat['Time'], format='%H:%M:%S').dt.hour
        # if time between 7:00 - 9:00, set to 'Morning-Peek'
        # if time between 16:00 - 19:00, set to 'Evening-Peek'
        # otherwise set to 'Off-Peek'
        dat['Time'] = dat['Time'].apply(lambda x: 'Morning-Peek' if 7 <= x <= 9 else (
            'Evening-Peek' if 16 <= x <= 19 else 'Off-Peek'))
        return dat
