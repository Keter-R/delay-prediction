...
import torch
from models.TPModels.GCN.GCN import GCN
...
class ST_MGCN(nn.Module):
    def __init__(self, M:int, seq_len:int, n_nodes:int, input_dim:int, lstm_hidden_dim:int, lstm_num_layers:int,
                 gcn_hidden_dim:int, sta_kernel_config:dict, gconv_use_bias:bool, gconv_activation=nn.ReLU, output_dim:int=1):
        super().__init__()
        self.M = M
        ...
        self.fc = nn.Linear(in_features=gcn_hidden_dim, out_features=output_dim, bias=True)
